    ...
    try {
      ...
      if (response.statusCode == 200 && response.body != null) {
        ...
        if (data is Map<String, dynamic>) {
          ...
          if (data['analysis_items'] != null &&
              data['analysis_items'] is List) {
            analysisItems.assignAll(data['analysis_items']);
          }

          if (data['career_recommendations'] != null &&
              data['career_recommendations'] is List) {
            careerRecommendations.assignAll(
              List<String>.from(
                data['career_recommendations'].map((e) => e.toString()),
              ),
            );
          }

          print(
            "=== AI INSIGHT: Parse berhasil! Score = ${overallScore.value} ===",
          );
        } else {
          errorMessage.value = "Format respons tidak dikenali";
          print(
            "=== AI INSIGHT: Body bukan Map! Tipe: ${data.runtimeType} ===",
          );
        }
      } else if (response.body != null && response.body['message'] != null) {
        errorMessage.value = response.body['message'];
        print(
          "=== AI INSIGHT: Gagal dengan pesan! status=${response.statusCode}, message=${response.body['message']} ===",
        );
      } else {
        errorMessage.value = "Server merespon: ${response.statusCode}";
        print(
          "=== AI INSIGHT: Gagal! status=${response.statusCode}, body=${response.body} ===",
        );
      }
    } catch (e, stackTrace) {
